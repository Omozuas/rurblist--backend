...
class SendEmails {
  ...
  static transporterCache = new Map();

  static getEmailTimeout() {
    const timeout = Number(process.env.EMAIL_TIMEOUT_MS);
    return Number.isFinite(timeout) && timeout > 0 ? timeout : 30000;
  }

  static getTransporterCacheKey(email, password) {
    return [
      ...
      Boolean(password || process.env.SMTP_PASSWORD),
    ].join(':');
  }

  static getTransporter(email, password) {
    const port = Number(process.env.SMTP_PORT || 587);
    const cacheKey = SendEmails.getTransporterCacheKey(email, password);

    if (SendEmails.transporterCache.has(cacheKey)) {
      return SendEmails.transporterCache.get(cacheKey);
    }

    const timeout = SendEmails.getEmailTimeout();
    ...
    SendEmails.transporterCache.set(cacheKey, transporter);
    return transporter;
  }

  static get verifyTransporter() {
    return SendEmails.getTransporter(process.env.EMAIL_VERIFY, process.env.EMAIL_VERIFY_PASSWORD);
  }

  static get supportTransporter() {
    return SendEmails.getTransporter(process.env.EMAIL_SUPPORT, process.env.EMAIL_SUPPORT_PASSWORD);
  }

  static get helloTransporter() {
    return SendEmails.getTransporter(process.env.EMAIL_HELLO, process.env.EMAIL_HELLO_PASSWORD);
  }
